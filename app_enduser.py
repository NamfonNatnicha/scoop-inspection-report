...
# App
# ----------------------------
st.set_page_config(page_title="Scoop Inspection (DEMO)", layout="wide")

# refresh every 3 seconds (no sleep)
st_autorefresh(interval=3000, key="refresh_3s")

st.title("🥄 Scoop Inspection (DEMO → Production-like Folder Watch)")
...
st.sidebar.divider()
st.sidebar.caption("DEMO: copy images into **incoming/** to simulate camera snapshots.")
...
latest_incoming = incoming_images[-1] if incoming_images else None

if st.session_state["run_mode"] and latest_incoming is not None:
    processed_latest_path = PROCESSED_DIR / latest_incoming.name

    # only process if not yet processed
    if not processed_latest_path.exists():
        try:
            _, dets = run_inference_and_save(
                model=model,
                img_path=latest_incoming,
            ...
            # alert (avoid repeating)
            alert_key = f"{now_str}|{latest_incoming.name}|{status}|{reason}"
            if alert_key != st.session_state.get("last_alert_key"):
                if status == "FAIL":
                    play_fail_beep()
                    popup_alert(reason)
                    st.session_state["last_alert_key"] = alert_key
                elif status == "NO_OBJECT" and alert_on_no_object:
                    popup_info("NO OBJECT detected")
                    st.session_state["last_alert_key"] = alert_key
...
with col_left:
    ...
    if latest_processed and is_image_file(latest_processed):
        st.image(safe_read_image(latest_processed), use_container_width=True, caption=latest_processed.name)
    else:
...
if RESULTS_CSV.exists():
    ...
    with c3:
        q = st.text_input("Search (filename / reason)", value="")
    ...
    if q.strip():
        qn = q.strip().lower()
        view = view[
            view["file"].astype(str).str.lower().str.contains(qn)
            | view["reason"].astype(str).str.lower().str.contains(qn)
        ]

    st.dataframe(view.tail(50), use_container_width=True)

    with st.expander("🖼️ View a record image"):
        if len(view) > 0:
            options = list(view.tail(200)["file"].astype(str).unique())
            sel = st.selectbox("Select file", options, index=len(options) - 1)
            img_path = PROCESSED_DIR / sel
            if img_path.exists():
                st.image(safe_read_image(img_path), use_container_width=True, caption=sel)
            else:
                st.info("Processed image not found (maybe cleared by reset).")
